...
public class BollavenditaSV {
	...
	public void postConstruct() {
		...
		listbolla = bollaB.getAllBollavendita();
	}

	public String printBollavendita() {
		GoldmanagerLogger.debug(BollavenditaSV.class, "printBollavendita init", GoldmanagerSession.getCurrentInstance().getUtente().getUsername());
		...
		try {
			gr.pdfReportFda(bolla, bolla.getBancometalli());
		} catch (JRException e) {
			GoldmanagerLogger.error(BollavenditaSV.class, "printBollavendita error jre", GoldmanagerSession.getCurrentInstance().getUtente().getUsername());
			e.printStackTrace();
		} catch (IOException e) {
			GoldmanagerLogger.error(BollavenditaSV.class, "printBollavendita error io", GoldmanagerSession.getCurrentInstance().getUtente().getUsername());
			e.printStackTrace();
		}
		return null;
	}
